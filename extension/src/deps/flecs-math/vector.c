...
void ecs_vec3_movetowards(
    EcsVec3 *vector_a,
    EcsVec3 *vector_b,
    float move_delta,
    EcsVec3 *out_position)
{
    float x = vector_a->x - vector_b->x;
    float y = vector_a->y - vector_b->y;
    float z = vector_a->z - vector_b->z;
    float distanceSqrt = x*x + y*y + z*z;

    if (distanceSqrt <= move_delta * move_delta || distanceSqrt == 0) {
        out_position = vector_b;
        return;
    }

    EcsVec3 direction;
    float distance = sqrtf(distanceSqrt);
    direction.x = (x / distance) * move_delta;
    direction.y = (y / distance) * move_delta;
    direction.z = (z / distance) * move_delta;

    ecs_vec2_add(vector_a, &direction, out_position);
}